...
class TestbedDatasetGE(InMemoryDataset):
    def __init__(self, root='/tmp', dataset='davis', 
                  xt_ge=None, transform=None,
                 pre_transform=None,smile_graph=None,saliency_map=False):

        #root is required for save preprocessed data, default is '/tmp'
        super(TestbedDatasetGE, self).__init__(root, transform, pre_transform)
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process( xt_ge, smile_graph)
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self, xd, xt_meth,  y, smile_graph):
        assert (len(xd)  == len(y)) and len(y) == len(xt_meth) , "The four lists must be the same length!"
        data_list = []
        data_len = len(xd)
