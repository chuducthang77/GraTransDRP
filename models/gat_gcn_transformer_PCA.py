...
class GAT_GCN_Transformer_PCA(torch.nn.Module):
    def __init__(self, n_output=1, num_features_xd=78, num_features_xt=25,
                 n_filters=32, embed_dim=128, output_dim=128, dropout=0.2, device=1):

        super(GAT_GCN_Transformer_PCA, self).__init__()
    ...
    def forward(self, data):
        ...
        # xt = self.fc1_xt(xt)
        if xt.size()[1] != 128 or xt.size()[0] != 128:
            pca = PCA(n_components=7)
        else:
            pca = PCA(n_components=self.output_dim)
        xt = pca.fit_transform(xt.cpu().detach().numpy())
        xt = torch.from_numpy(xt).to(0)
